...
fun transform(point : Point3D, c: DrawingContext) : Point2D {
    val k = c.camera.planeDistance / (point.y - c.camera.y)
    val newX = (k * point.x + c.scene.x)
    val newZ = (c.scene.z - k * point.z)
